...
<template>
  <div class="max-w-[816px] mx-auto">
    <BaseText
      :text="t('pages.course.create.files.title')"
      font="medium"
    ...
    >
      <div class="bg-red-50 border border-red-200 rounded-lg p-4 md:p-3 lg:p-4">
        <div class="flex">
          ...
          <div class="ml-3">
            <BaseText
              :text="t('pages.course.create.files.validationErrors')"
              color="error"
              :tone="600"
              font="medium"
              type="p-sm"
    ...
    <div v-if="uploadedFiles.length > 0" class="mb-4 md:mb-3 lg:mb-4">
      <div class="flex justify-between items-center text-sm md:text-xs lg:text-sm text-gray-600">
        <span>{{
          t('pages.course.create.files.fileCount', { count: uploadedFiles.length, max: 10 })
        }}</span>
        <span>{{
          t('pages.course.create.files.totalSize', {
