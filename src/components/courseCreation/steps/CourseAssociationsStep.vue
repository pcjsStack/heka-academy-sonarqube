...
import AssociationDetailsExpanded from './AssociationDetailsExpanded.vue'
...
const associationTab = ref<'courses' | 'lessons' | 'files' | 'quizzes'>('courses')
...
const searchQuery = ref({ courses: '', lessons: '', files: '', quizzes: '' })

type AssociationItem = {
  id: string | number
  type: 'course' | 'lesson' | 'file' | 'quiz'
  title: string
  iconBg: string
  iconName: Icons
  iconColor: 'success' | 'primary' | 'error' | 'warning' | 'neutral'
  isNew?: boolean
  sourceList?: 'courses' | 'lessons' | 'files' | 'quizzes'
  thumbUrl?: string
  status?: string
  visibility?: string
...
// Initialize leftItems from editCourse assignations
const initializeFromEditCourse = () => {
  if (!props.editCourse?.assignations || hasInitializedFromEdit.value) return

  const assignationItems: AssociationItem[] = props.editCourse.assignations.map(
    (assignation: Assignation) => {
      const relatedType = assignation.relatedType.toLowerCase()

      if (relatedType === 'course') {
        return mapCourse(assignation.model as unknown as CourseItem)
      } else if (relatedType === 'lessons') {
...
const loadCourses = async (reset = false) => {
  ...
  try {
    ...
    // Filter out items that are already in leftItems and exclude current course
    const newItems = items
      .filter((item: CourseItem) => item.id.toString() !== (props.courseId?.toString() as string))
      .map(mapCourse)
...
// Filter out items that are already in the left side and exclude current course
const rightList = computed<AssociationItem[]>({
  get() {
    ...
    // Filter out items that are already in leftItems
    let filtered = list.filter((item) => !leftItemsIds.value.has(item.id))

    // Additionally filter out the current course being edited (if in courses tab)
    if (associationTab.value === 'courses' && props.courseId !== undefined) {
      filtered = filtered.filter((item) => item.id.toString() !== props.courseId?.toString())
    }
...
const toggleExpanded = async (
  ...
  itemType: 'course' | 'lesson' | 'file' | 'quiz',
) => {
  ...
  if (itemType === 'course' || itemType === 'lesson' || itemType === 'quiz') {
    ...
    try {
      ...
      if (itemType === 'course') type = CourseExecutionType.COURSE
      else if (itemType === 'lesson') type = CourseExecutionType.LESSON
