...
import { createEditSkillPayload } from '@/utils/badgeSkillsPayload'
...
import type {
  ...
  GetSkillsParams,
  StoredAssociations,
  ExtendedSkillsAssociations,
} from '@/types/BadgeAndSkill'

interface Props {
  show: boolean
  skillId?: number
  initialSkillsData?: Partial<SkillsFormData>
  reloadParams?: GetSkillsParams
}
...
const emit = defineEmits<{
  close: []
  update: [skillId: number, skillsData: SkillsFormData]
}>()

const showSuccessScreen = ref<boolean>(false)

// Store full association objects from initialSkillsData
...
// Computed validation
const isSaveEnabled = computed(() => skillsValidation.value.isFormValid)

// Association options - add missing IDs from skillsData.associations
const associationOptions = computed(() => {
  ...
  // Add missing IDs from skillsData.associations using stored full objects
  const selectedFileIds = new Set(skillsData.value.associations.fileIds || [])
  const selectedCourseIds = new Set(skillsData.value.associations.courseIds || [])
  const selectedLessonIds = new Set(skillsData.value.associations.lessonIds || [])
  const selectedQuizIds = new Set(skillsData.value.associations.quizIds || [])
...
watch(
  () => props.initialSkillsData,
  async (newData) => {
    if (newData) {
      // Store full association objects if they exist (from editingSkillsData.associations)
      const associations = newData.associations as ExtendedSkillsAssociations | undefined
      ...
      // For editing, ensure only single name is used
      const skillNames = newData.names || ['']
      const singleName = skillNames.length > 0 && skillNames[0] ? skillNames[0] : ''
      skillsData.value = {
        names: [singleName],
...
const handleUpdate = async () => {
  if (props.skillId) {
    try {
      // Build skill payload for editing (single name only)
      const payload = createEditSkillPayload(skillsData.value)

      // Call the API to update the skill with reload params
      await badgeStore.editSkill(props.skillId, payload, props.reloadParams)

      // Emit update event for parent to handle
      emit('update', props.skillId, skillsData.value)
...
<template>
  ...
  <BaseSideModal
    v-else-if="show"
    :title="t('pages.badgeSkills.edit.skillTitle')"
    size="md"
    :close-button="true"
    custom-class="left-0"
    :z-index="9999999"
