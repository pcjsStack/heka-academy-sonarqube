...
import { ParticipantType } from '@/types/Course'
...
const loadUsers = async () => {
  ...
  try {
    ...
    const newItems = items
      .map((item: UserApiItem, idx: number) => mapUser(item, startIndex + idx))
      .filter((item: ParticipantItem) => !leftIds.has(item.id))
...
const loadGroups = async () => {
  ...
  try {
    ...
    const leftIds = new Set(leftItems.value.map((item) => item.id))
    // Filter out items that are already in leftItems
    const newItems = items.map(mapGroup).filter((item: ParticipantItem) => !leftIds.has(item.id))
...
const loadStores = async () => {
  ...
  try {
    ...
    const leftIds = new Set(leftItems.value.map((item) => item.id))
    // Filter out items that are already in leftItems
    const newItems = items.map(mapStore).filter((item: ParticipantItem) => !leftIds.has(item.id))
...
const loadClusters = async () => {
  ...
  try {
    ...
    const leftIds = new Set(leftItems.value.map((item) => item.id))
    // Filter out items that are already in leftItems
    const newItems = items.map(mapCluster).filter((item: ParticipantItem) => !leftIds.has(item.id))
...
const loadRoles = async () => {
  ...
  try {
    ...
    const leftIds = new Set(leftItems.value.map((item) => item.id))
    // Filter out items that are already in leftItems
    const newItems = items.map(mapRole).filter((item: ParticipantItem) => !leftIds.has(item.id))
          ...
          p.type === 'users'
            ...
            : p.type === 'groups'
              ...
              : p.type === 'stores'
                ...
                : p.type === 'clusters'
                  ? ParticipantType.CLUSTER
                  : ParticipantType.ROLE,
        title: p.fullName || p.name || p.title || `ID: ${p.id}`,
  ...
  <div class="w-full">
    ...
    >
      <div class="flex items-end">
        <BaseText
          :text="t('pages.quizCreation.participants.thisCourse')"
          :tone="900"
          color="neutral"
          font="semibold"
          type="p-lg"
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 h-[calc(100vh-250px)]">
        ...
        <div class="flex-1 overflow-y-auto no-scrollbar">
          ...
          >
            ...
            >
              <div class="flex items-center gap-3 md:gap-2.5 lg:gap-3">
                ...
                <div>
                  <BaseText
                    ...
                    color="neutral"
                    type="p-sm"
                    class="text-xs md:text-[11px] lg:text-xs"
                  />
                  <BaseText
              ...
              <div class="flex items-center gap-1 md:gap-0.5 lg:gap-1">
                <BaseButtonIcon
                  icon="arrow-up"
                  color="neutral"
      ...
      <div
        class="lg:col-span-1 lg:border-l lg:border-neutral-200 lg:pl-5 pt-4 md:pt-3 lg:pt-4 h-full flex flex-col"
      >
        <!-- Mobile-only tabs above right content -->
        <div class="block lg:hidden px-2 md:px-3 lg:px-2 mb-3 md:mb-2.5 lg:mb-3">
          <BaseTab :tabs="tabs" v-model="participantTab" />
        </div>
                ...
                <div class="flex items-center gap-3 md:gap-2.5 lg:gap-3">
                  ...
                  <div>
                    <BaseText
                      ...
                      :tone="900"
                      color="neutral"
                      class="text-sm md:text-[13px] lg:text-sm"
                    />
                    <BaseText
                      v-if="element.subtitle"
                      :text="element.subtitle"
