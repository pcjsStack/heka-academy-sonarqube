...
import SetRecurrence from '@/components/courseCreation/steps/SetRecurrence.vue'
...
// Form data - Restriction Access - Initialize with props.editLesson.rules if editing
const accessRestrictions = ref<Restriction[]>(
  props.editLesson?.rules ? [...props.editLesson.rules] : [...props.restrictions],
)

// Form data - Completion Conditions - Initialize from editLesson if editing
const completionCondition = ref(props.editLesson?.restrictions?.completionCondition || '')

// Form data - Set Recurrence - Initialize from editLesson if editing
...
// Initialize recurrence data from existing lesson
const initializeRecurrenceFromLesson = () => {
  if (!props.editLesson?.restrictions) return

  const restrictions = props.editLesson.restrictions
...
onMounted(async () => {
  ...
  // Initialize form data from editLesson when editing
  initializeRecurrenceFromLesson()
})

// Watch for editLesson changes
watch(
  () => props.editLesson,
  () => {
    if (props.editLesson) {
      // Update access restrictions from rules
      if (props.editLesson.rules) {
        accessRestrictions.value = [...props.editLesson.rules]
      }

      // Update completion condition
      if (props.editLesson.restrictions?.completionCondition) {
        completionCondition.value = props.editLesson.restrictions.completionCondition
      }
