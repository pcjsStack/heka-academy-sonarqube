...
const emit = defineEmits<{
  (e: 'close'): void
  (e: 'applyFilters', filters: GetLessonsParams): void
  (e: 'clear'): void
  (e: 'update:filters', filters: GetLessonsParams): void
}>()

const props = defineProps<{
  filters: GetLessonsParams
  isAdmin: boolean
}>()
...
const activeFilterCount = computed(() => {
  ...
  if (props.isAdmin) {
    // Admin filters
    if (props.filters.status === LessonStatus.PUBLISHED) count++
...
const handlePublishedChange = (value: boolean) => {
  emit('update:filters', {
    ...props.filters,
    status: value ? LessonStatus.PUBLISHED : undefined,
  })
}
      ...
      <div class="flex flex-col gap-6 overflow-y-auto flex-1 h-full min-h-0">
        ...
        <div v-if="isAdmin">
          ...
          <div class="flex items-center gap-3">
            <BaseCheckbox
              :model-value="[filters.status === LessonStatus.PUBLISHED ? 'published' : '']"
              value="published"
