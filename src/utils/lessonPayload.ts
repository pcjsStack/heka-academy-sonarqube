...
import type { Restriction, ParticipantShapePayload } from '@/types/Course'

type SettingsShape = Record<string, unknown>

type AssociationItem = {
  id: number | string
  type: 'lesson' | 'file' | 'quiz' | string
  isNew?: boolean
}
...
export interface CreateLessonPayloadShape {
  name: string
  shortName: string
  ...
  assignations: Array<{
    index: number
    type: 'file_asset' | 'new_file' | 'lessons' | 'quiz'
    id: number | string
  }>
...
export function buildCreateLessonPayload(params: {
  ...
  const fileIds = left
    .filter(
      (i) =>
        (i.type.toLowerCase() === 'file' || i.type.toLowerCase() === 'file_asset') &&
        !i.isNew &&
  ...
  const base: CreateLessonPayloadShape = {
    name,
    shortName,
    categoryId,
    visibility,
