        ...
        <div class="px-4 sm:px-6 md:px-4 lg:px-8 pt-4 sm:pt-4 md:pt-3 lg:pt-4">
          <div class="flex justify-between items-center mb-8 md:mb-6 lg:mb-8">
            ...
            <BaseCircularProgress
              ...
              :label="t('pages.lessonDetails.overallProgress')"
              color="primary"
              size="md"
            />
          </div>
        ...
        <div class="">
          <div class="space-y-2">
            <BaseCourseItemCard
              ...
              @click="handleItemClick(item as LessonAssignations)"
            />
          </div>
        </div>
      </div>
...
import LessonCompletedModal from '@/components/modal/LessonCompletedModal.vue'
...
// Define LessonAssignations interface similar to CourseAssignations
interface LessonAssignations {
  id: number
  title: string
...
const overallProgress = computed(() => {
  return lessonsStore.getLessonDetails?.execution?.percentage || 0
})

// Check if lesson is in maintenance mode
const isMaintenance = computed(() => {
  return (
    lessonsStore.getLessonDetails?.visibility === VisibilityStatus.MAINTENANCE ||
    lessonsStore.getLessonDetails?.visibility === 'maintenance'
  )
})
const showLessonCompletedModal = ref(false)
...
// Check if lesson has assignations
const hasAssignations = computed(() => {
  return (lessonsStore.getLessonDetails?.assignations?.length || 0) > 0
})
...
const handleBeforeUnload = (event: BeforeUnloadEvent) => {
  ...
  if (!isTimerStopped.value && timerDetails.value?.running) {
    ...
    if (lessonsStore.getLessonDetails?.execution?.timerId) {
      // Use special unload method that ensures delivery even after page closes
      TimerService.stopTimerOnUnload(lessonsStore.getLessonDetails.execution.timerId)
...
const loadLesson = async (id: number) => {
  await lessonsStore.fetchLessonById(id)

  // Check if lesson has assignations - if not, don't load timer details
  if (!hasAssignations.value) {
    return
  }

  // Fetch timer details if timerId exists and lesson has assignations
  if (lessonsStore.getLessonDetails?.execution?.timerId) {
    try {
      isInitializingTimer.value = true // Set flag to prevent API calls during initialization

      const timerResponse = await TimerService.getTimer(
        lessonsStore.getLessonDetails.execution.timerId,
      )
...
watch(
  ...
  async (newId) => {
    ...
    showLessonCompletedModal.value = false
    ...
    await loadLesson(newId as unknown as number)
...
const handleTimerStart = async (fromEvent: boolean = true) => {
  ...
  if (!isInitializingTimer.value && lessonsStore.getLessonDetails?.execution?.timerId) {
    try {
      await TimerService.startTimer(lessonsStore.getLessonDetails.execution.timerId)
      if (timerDetails.value) {
...
const handleTimerStop = async (fromEvent: boolean = true) => {
  ...
  if (!isInitializingTimer.value && lessonsStore.getLessonDetails?.execution?.timerId) {
    try {
      await TimerService.stopTimer(lessonsStore.getLessonDetails.execution.timerId)
      if (timerDetails.value) {
        timerDetails.value.running = false
      }
    } catch (error) {
...
const handleCloseLessonCompletedModal = () => {
  showLessonCompletedModal.value = false
}
...
const handleItemClick = (item: LessonAssignations) => {
  // Allow clicks if lesson is 100% complete, otherwise prevent if timer is stopped
  ...
  // TODO: Handle other action types (courses, lessons, etc.) when needed
}
...
const buildExecutionPayload = (percentage?: number): CourseExecution => {
  ...
  return {
    ...
    ownerType: CourseExecutionType.LESSON,
...
const handleCloseFileViewer = async (payloadData?: {
  ...
  // If lesson is 100% complete, don't call execution API - just cleanup
  ...
  await lessonsStore.lessonExecution(payload, route.params.id as unknown as number)
  cleanup()
}
